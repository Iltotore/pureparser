...
inline def compileTimeProperty(name: String): Option[String] = ${ compileTimePropertyImpl('name) }

def compileTimePropertyImpl(name: Expr[String])(using Quotes): Expr[Option[String]] =
  Expr(sys.props.get(name.valueOrAbort))