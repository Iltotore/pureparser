...
object Parser:
  ...
  def repeatDiscard[I](parser: Parser[I, Any]): Parser[I, Unit] =
    if Parser.isEOF then ()
    else if Parser.option(parser).isEmpty then ()
    else Parser.repeatDiscard(parser)
  ...
  def separatedBy[I, A](parser: Parser[I, A], separator: Parser[I, Unit]): Parser[I, List[A]] =
    ...
    @tailrec
    def rec(accumulator: List[A]): Parser[I, List[A]] =
      Parser.option(separator) match
        case Some(_) => rec(accumulator :+ parser)
        case None    => accumulator
  ...
  def separatedByReduce[I, A](parser: Parser[I, A], separator: Parser[I, (A, A) => A]): Parser[I, A] =
    ...
    def rec(accumulator: A): Parser[I, A] =
      Parser.option(separator) match
        case Some(reduce) => rec(reduce(accumulator, parser))
        case None         => accumulator
